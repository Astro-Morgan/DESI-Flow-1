"""Training + evaluation script for DirectBOOST (the direct photometry ->
redshift baseline, no latent space).

Builds SDSS+WISE magnitudes from fluxes, filters to the same "golden sample"
used everywhere else in this project, trains the ugrizW configuration, and
evaluates it in FORCED mode (regression quality in isolation) and end-to-end
AUTOMATIC mode (classifier + expert) -- strictly on the held-out 10% test
split in both cases.

If MODEL_FILE already exists, training is skipped and the cached model is
loaded directly. NOTE: the manuscript's quoted AUTOMATIC-mode outlier rates
(0.91% GALAXY / 9.84% QSO) were reproduced from a run that evaluated over the
*full* golden sample (train+test combined) rather than the held-out split --
that is a data-leakage bug in how those numbers were generated, not a
property of the model itself. This script evaluates strictly on X_te/z_te,
so the numbers it reports are the honest out-of-sample performance and will
not match the manuscript's cited figures; treat these as the corrected
...
from DirectBOOST import DirectBOOST

# --- CONFIGURATION ---
DATA_FILE = 'merged_filtered_sample.hdf5'   # <-- UPDATE: catalog HDF5
EMB_FILE = 'embeddings.h5'                  # <-- UPDATE: spectral embeddings HDF5 (ground-truth labels only)

# The cached checkpoint referenced by the manuscript's DirectBOOST comparison.
MODEL_FILE = 'DirectBOOST.pkl'
...
def load_and_prep_data():
    ...
    Embeddings are loaded only to define the ground-truth GALAXY/QSO labels
    used for evaluation (DirectBOOST's own training never sees them).
    """
...
def _load_directboost_compat(path):
    """joblib.load with a fallback for DirectBOOST/DirectPipeline pickled
    from a script's own __main__ namespace.

    Earlier runs (e.g. DirectBOOST_Reference.py) defined these classes
    directly at module scope in a script executed as __main__, so objects
    saved from that run record __module__ == '__main__'. Now that the
    classes live in the standalone DirectBOOST module, try a plain load
    first and only patch sys.modules['__main__'] if that fails.
    """
    try:
    ...
    import DirectBOOST as _mod
    main_mod = sys.modules['__main__']
    if not hasattr(main_mod, 'DirectBOOST'):
        main_mod.DirectBOOST = _mod.DirectBOOST
    if not hasattr(main_mod, 'DirectPipeline'):
        main_mod.DirectPipeline = _mod.DirectPipeline
    sys.modules.setdefault('DirectBOOST_Reference', _mod)
    return joblib.load(path)
...
if __name__ == "__main__":
    ...
    if os.path.exists(MODEL_FILE):
        ...
        model = _load_directboost_compat(MODEL_FILE)
    else:
        print("\nNo cached model found. Initializing DirectBOOST Training...")
        model = DirectBOOST()
    ...
    for mode_name, z_pred_mode in [('Forced', z_pred_forced_all), ('Automatic', z_pred_auto)]:
        ...
        fig.suptitle(f"DirectBOOST Baseline (ugrizW) - {mode_name.upper()} | "
                     f"$R^2$: {g_r2:.4f}, $\\sigma_{{NMAD}}$: {g_nmad:.4f}", y=0.9)

        out_name = os.path.join(OUT_DIR, f"DirectBOOST_{mode_name}_Photometric_Redshift.png")
