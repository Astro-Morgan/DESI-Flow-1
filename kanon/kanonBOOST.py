...
Supports the ugrizW photometric configuration only (u, g, r, i, z, W1, W2):
    ...
    2. A per-class ``kanonPipeline`` runs an iteratively-stacked proxy-z
       ...
       then maps [features, proxy_z] -> 32-dim embedding with a native
       multi-output XGBoost regressor (a single ``XGBRegressor`` fit
       directly on the 32-column target, not a per-dimension wrapper)
       trained on out-of-fold proxy predictions (to avoid leakage).
...
Class and module names are held fixed (``kanonBOOST`` / ``kanonPipeline`` in
``kanonBOOST.py``) so that ``joblib``/``pickle`` can still resolve cached
``.pkl`` files trained under this naming; do not rename without also
updating or aliasing every cached checkpoint that depends on it.
"""
...
class kanonPipeline:
    ...
    Two stages:
        ...
        latent_mapper: a native multi-output XGBoost regressor -- one
            ``XGBRegressor`` fit directly on the full 32-column embedding
            target, not wrapped per-dimension -- mapping
    ...
    def train(self, X_mags, z_true, embeddings, iterations=5, n_folds=5):
        ...
        # Native multi-output: one XGBRegressor fit directly on all 32
        # embedding dimensions at once (no MultiOutputRegressor wrapper).
...
class kanonBOOST:
    ...
    ``kanonPipeline`` for the ugrizW configuration, and a single shared k-NN
    "manifold memory" (embeddings -> z) fitted on the spectral training set.
    ``predict`` expects exactly 7 columns (u, g, r, i, z, W1, W2); any other
